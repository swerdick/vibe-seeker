...
probably like. Relying on tools that suggest specific acts isn't helpful because you've probably never heard of
...
music that you like to listen to maybe we can give you the confidence to go check it out regardless of who's playing. Or
...
2. The app reads your top artists and fetches tags for each from Last.fm (with Ticketmaster classifications as fallback)
3. NYC venue data is fetched from Ticketmaster
4. Each venue gets a vibe profile built from its booking history and upcoming shows
5. Venue vibes are scored against your vibe using cosine similarity
...
Create developer accounts and/or "apps" in Spotify, Last.fm, and Ticketmaster, then:

- Obtain the client IDs, client secrets, and API keys for each app.
- In the Spotify app settings, set the redirect/callback URI to `http://127.0.0.1:5173/callback`.
